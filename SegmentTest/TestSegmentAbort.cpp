                    ...
                    void *Extension)
{
    ...
    if (AbortCountDown > AbortData->AbortCount){
        sprintf(Buffer, "AbortStartCallback Condition FALSE - Attaching to next segment - abort at %d (cur %d)", 
        ...
        sprintf(Buffer, "AbortStartCallback Condition TRUE - Skipping next segment (%d <= %d)", 
                    ...
                    void *Extension)
{
    ...
    if (AbortCountDown > AbortData->AbortCount){
        sprintf(Buffer, "AbortRunCallback at %d (cur %d)", AbortData->AbortCount, AbortCountDown);
        UtilTraceExecution(Buffer);
        printf("a");
        AbortCountDown -= 1;
        ...
        sprintf(Buffer, "AbortRunCallback Condition TRUE - Aborting next segment (%d <= %d)", 
                        AbortCountDown, AbortData->AbortCount);
        printf("\n%s\n", Buffer);
        UtilTraceExecution(Buffer);
                        ...
                        void *UserData)
{
    ...
    if (AbortData->AbortCount < 0){
        sprintf(Buffer, "AbortValidateCallback %d INVALID", AbortData->AbortCount);
        UtilTraceExecution(Buffer);
        printf("%s\n", Buffer);
		return SeInvalid;
    }
    else{
        sprintf(Buffer, "AbortValidateCallback %d VALID", AbortData->AbortCount);
        UtilTraceExecution(Buffer);
        return SeSuccess;
    }
}
...
static char *AbortUserData = "ABORT-USER-DATA";


void
RegisterAbortSegment(ENGINE_HANDLE Engine)
{
    SegmentCommand_t Command = {

        TestAbort,              //  From CiiMessageData.<Type>.SubCommand     
